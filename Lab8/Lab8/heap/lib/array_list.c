// get from the Lab5 solution provided by Robert used for the lib of Lab8
// one function swqp() is added
...
// swap 2 items
bool alist_swap(ArrayList* alist, int index1, int index2){
	if (alist == NULL) { return false; }
	if (index1 < 0 || index2 < 0 || index1 >= alist->size || index2 >= alist->size) { return false; }

	// swqp
	void* element = alist_get(alist, index1);
	alist->arr[index1] = alist->arr[index2];
	alist->arr[index2] = element;
